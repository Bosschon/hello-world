...
while (isNaN(u) == true || u <= 0) {
    u = prompt("ความเร็วลูกบอล (หน่วยเมตร/วินาที)")
    if (isNaN(u) == false && u > 0) { break; }
}
let angle = prompt("มุมกี่องศา")
while (isNaN(angle) == true || angle <= 0) {
    angle = prompt("มุมกี่องศา")
    if (isNaN(angle) == false && angle > 0) { break; }
}
let s = prompt("ระยะห่าง (หน่วยเมตร)")
while (isNaN(s) == true || s <= 0) {
    s = prompt("ระยะห่าง (หน่วยเมตร)")
    if (isNaN(s) == false && s > 0) { break; }
}
var g = -9.81
...
if (sy2_fixed > 2.44) {
    ...
    if (t2_fixed < 0.5 && sx > s) { document.getElementById("result3").innerHTML = "ลูกบอลเข้าประตู" }
    else if (t2_fixed > 0.5 && sx > s) { document.getElementById("result3").innerHTML = "ไม่เข้า ผู้รักษาประตูรับได้" }
    else if (sx < s) { document.getElementById("result3").innerHTML = "ไม่เข้า ลูกบอลตกพื้นก่อน" }
}
